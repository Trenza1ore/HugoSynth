% The psola function found in 
% http://users.cs.cf.ac.uk/Dave.Marshall/Multimedia/Lecture_Examples/Synthesis/Granular.zip
% Tidied up by a bit for readability
function out = psola(in, m, alpha, beta)
    ...
    out = zeros(Lout, 1); %output signal
    ...
    while round(tk)<Lout
      ...
      out(iniGr:endGr, 1) = out(iniGr:endGr, 1) + gr; %overlap new segment
      tk = tk + pit / beta;
    end
end